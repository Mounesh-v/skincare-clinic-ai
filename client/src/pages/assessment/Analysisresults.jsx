                  ...
                  <div className="p-6 lg:p-8">
                    <div className="grid gap-8 lg:grid-cols-2">
                      ...
                      <div className="space-y-5">
                        ...
                        {analysisResults.conditionContributions?.length > 0 && (
                          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <div>
                                <h4 className="text-sm font-bold text-slate-800">What It Means</h4>
                                <p className="text-xs text-slate-500">How each condition contributed to your result</p>
                              </div>
                            </div>
                            <div className="space-y-3">
                              {analysisResults.conditionContributions.map((contrib) => {
                                ...
                                const condIcon = contrib.condition === 'Wrinkles' ? '〰️'
                                  : contrib.condition === 'Pores' ? '🔵'
                                  : contrib.condition === 'Acne' ? '🔴'
                                  : contrib.condition === 'Blackheads' ? '⚫'
                                  : contrib.condition === 'Dark Spots' ? '🟤' : '📍';
                                return (
                                  <div key={contrib.condition} className="flex gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                                        <span className="text-xs font-bold text-slate-800">{contrib.condition}</span>
                                        <span className="text-xs text-slate-400">{pct}% detected</span>
                                        <span className={`text-xs font-semibold px-2 py-px rounded-full border ${tagColor}`}>→ {contrib.points_to}</span>
                                      </div>
                                      <div className="h-1 rounded-full bg-slate-200 overflow-hidden mb-1.5">
                                        <div className="h-full rounded-full bg-gradient-to-r from-purple-400 to-pink-400" style={{ width: `${pct}%` }} />
                                      </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                          <div className="flex items-center gap-2 mb-4">
                            <span className="w-7 h-7 bg-teal-100 rounded-full flex items-center justify-center text-sm">📊</span>
                            <div>
                              <h4 className="text-sm font-bold text-slate-800">Skin Type Breakdown</h4>
                              <p className="text-xs text-slate-500">Model confidence across all skin types</p>
                            </div>
                          </div>
                          <div className="space-y-3">
                            {(() => {
                              const sorted = SCORE_KEYS
                                .map(k => ({ key: k, ...SKIN_TYPE_CONFIG[k], score: analysisResults.predictionScores[k] ?? 0 }))
                                .sort((a, b) => b.score - a.score);
                              return sorted.map((item, idx) => {
                                const pct = Math.round(item.score * 100);
                                const rank = idx === 0 ? { label: 'Primary', badge: 'bg-emerald-100 text-emerald-700' }
                                  : idx === 1 ? { label: 'Secondary', badge: 'bg-slate-100 text-slate-600' }
                                  : null;
                                return (
                                  <div key={item.key} className="space-y-1.5">
                                    <div className="flex items-center justify-between text-sm">
                                      <span className="flex items-center gap-2 font-semibold text-slate-700">
                                        {item.label}
                                        {rank && (
                                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${rank.badge}`}>{rank.label}</span>
                                        )}
                                      </span>
                        ...
                        {featureInsights.length > 0 && (
                          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="flex items-center gap-2 mb-4">
                              <span className="w-7 h-7 bg-sky-100 rounded-full flex items-center justify-center text-sm">📷</span>
                              <div>
                                <h4 className="text-sm font-bold text-slate-800">Image Quality</h4>
                                <p className="text-xs text-slate-500">How your photo affected analysis quality</p>
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                              {featureInsights.slice(0, 4).map((insight) => {
                                const isGood = insight.label.toLowerCase().includes('brightness')
                                  ? insight.value > 100
                                  : insight.label.toLowerCase().includes('oil')
                                  ? insight.value < 300
                                  : true;
                                return (
                                  <div key={insight.label} className="rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-1">
                                    <p className="text-xs uppercase tracking-wide text-slate-500 font-semibold">{insight.label}</p>
                            ...
                            <p className="text-xs text-slate-400 mt-3">
                              Use natural daylight for highest accuracy. Avoid flash or heavy filters.
                            </p>
                          </div>
                        )}
                        ...
                        {analysisResults.imageNotes && (
                          <div className="rounded-xl bg-sky-50 border border-sky-200 p-4">
                            <div className="flex gap-3">
                              <span className="text-sky-600 text-lg flex-shrink-0 mt-0.5">💬</span>
                              <div>
                                <p className="text-xs font-bold text-sky-900 mb-1 uppercase tracking-wide">AI Observation</p>
                                <p className="text-sm text-sky-800 leading-relaxed">{analysisResults.imageNotes}</p>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm">
                          <h4 className="text-sm font-bold text-slate-800 mb-3">Next Steps</h4>
                          <div className="flex flex-col gap-2">
                            ...
                            <button
                              onClick={() => {
                                const el = document.getElementById('skincare-plan');
